...
export class AccessComponent implements OnInit {
  urlObservatory: string;
  urlObservatorySafe: SafeResourceUrl;
  ...
  constructor(
    private apiService: ObservatoryApiService,
    ...
    if (!environment.production) email = 'talentoo-desarrollo@talenttools.es';

    const password = this.generatePassword(email);

    this.checkParams = this.route.snapshot.queryParams;
    this.urlObservatory = 'https://blog.kolete.es/blog?tokenTT=';
  ...
  navigateFlagHelper(data) { // si tiene params inicia sesión luego redirige a la ruta de los parámetros
    this.urlObservatory += data.user.tokenTT + '&embed=true&menu=true&lang=' + window.sessionStorage.getItem('lang') + '&primaryColor=' + this.config.company.primaryColor.substr(1) + '&secondaryColor=' + this.config.company.secondaryColor.substr(1);
    this.urlObservatorySafe = this.sanitizer.bypassSecurityTrustResourceUrl(this.urlObservatory);
    this.finishLoading = true;
  }
  ...
  redirectIframe() {
    ...
    this.urlObservatory = `https://app.kolete.es/${this.checkParams.urlIframe}?embed=true&url=${this.checkParams.urlIframe}`;
    this.urlObservatorySafe = this.sanitizer.bypassSecurityTrustResourceUrl(this.urlObservatory);
    this.finishLoading = true;
  }
